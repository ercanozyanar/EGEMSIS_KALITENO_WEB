...
{
    public class OfsetBitirmeController : Controller
    {
        ...
        public ActionResult Ofsetbitirmelist(int sayfa = 1)
        {
            var degerler = db.EGEM_OFSET_BITIRME_KALITE.ToList().ToPagedList(sayfa, 20);
            return View(degerler);
        }
        public ActionResult Ofsetbitirmeduzen(string Search_Data, string Filter_Value, int? Page_No)
        {
            if (Search_Data != null)
            {
                Page_No = 1;
            ...
            var students = from stu in db.EGEM_OFSET_BITIRME_KALITE select stu;
        ...
        [HttpGet]
        public ActionResult Ofsetbitirmeekle()
        {
        ...
        [HttpPost]
        public ActionResult Ofsetbitirmeekle(EGEM_OFSET_BITIRME_KALITE p1)
        {
            List<SelectListItem> degerler1 = (from i in db.EGEM_KALITE_PERSONEL.ToList()
                                              select new SelectListItem
                                              {
            ...
            db.EGEM_OFSET_BITIRME_KALITE.Add(p1);
            db.SaveChanges();
            return View();
        }
        [HttpPost]
        ...
        public ActionResult SendEmail(string MSIPARIS_NO, string MBOBINNO, string MHATA, string MIMAJ)
        {
            try
            {
                ...
                komut.CommandText = "INSERT INTO EGEM_OFSETBITIRME_KALITENOKIMAJ (SIPARISNO,BOBINNO,HATA,IMAJ,ADRES) VALUES (@MSIPARIS_NO,@MBOBINNO,@MHATA,@IMAJ,@ADRES)";
        ...
        {
            var bitirme = db.EGEM_OFSET_BITIRME_KALITE.Find(id);
            db.EGEM_OFSET_BITIRME_KALITE.Remove(bitirme);
            db.SaveChanges();
            return RedirectToAction("Ofsetbitirmeduzen");
        ...
        public FileResult Export()
        {
            ...
            var customers = from customer in entities.EGEM_OFSET_BITIRME_KALITE.Take(10)
                            select customer;

            foreach (var customer in customers)
            {
                dt.Rows.Add(customer.TARIH, customer.SIPARISNO, customer.STOK_KODU, customer.STOK_ADI, customer.PROSES);
            }
